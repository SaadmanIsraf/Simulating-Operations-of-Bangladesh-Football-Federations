...
public class player_medical_status_controller {
    ...
    public void initialize() {
        ...
        match_day_column.setCellValueFactory(new PropertyValueFactory<>("declarationDate"));
        declaration_status_column.setCellValueFactory(new PropertyValueFactory<>("declarationStatus"));
    ...
    public void save_button_on_action(ActionEvent actionEvent) {
        ...
        String declarationStatus = gameplay_declaration_combobox.getValue();
        ...
        player_medical_fitness_tableview.getItems().setAll(playerDeclarations);
        player_medical_fitness_tableview.refresh();

        displayPlayerAndDeclarations(foundPlayer, playerDeclarations);

        player_medical_fitness_tableview.getSelectionModel().select(declaration);
        player_medical_fitness_tableview.scrollTo(declaration);
    ...
    public void search_button_on_action(ActionEvent actionEvent) {
        ...
        if (foundPlayer == null) {
            ...
            player_medical_fitness_tableview.getItems().clear();
        ...
        if (playerDeclarations.isEmpty()) {
            ...
            MedicalDeclaration latestDeclaration =
                    playerDeclarations.get(playerDeclarations.size() - 1);

            player_medical_fitness_tableview
                    .getSelectionModel()
                    .select(latestDeclaration);

            player_medical_fitness_tableview.scrollTo(latestDeclaration);
    ...
    private RehabilitationProgress findLatestRehabilitationProgress(int playerId) {
    ...
    private List<MedicalDeclaration> getDeclarationsForPlayer(int playerId) {
    ...
    private int generateDeclarationId() {
        ...
        for (MedicalDeclaration declaration :
            ...
            if (declaration.getDeclarationId() > highestId) {
                highestId = declaration.getDeclarationId();
            }
        }
    ...
    private void displayPlayerAndDeclarations(
        ...
        details.append("=====================================================\n");
        details.append("Player ID          : ").append(player.getId()).append("\n");
        details.append("Player Name        : ").append(player.getName()).append("\n");
        details.append("Team               : ").append(player.getTeamName()).append("\n");
        details.append("Playing Position   : ").append(player.getPlayingPosition()).append("\n");
        details.append("Age                : ").append(player.getAge()).append("\n");
        details.append("Fitness Status     : ").append(player.getFitnessStatus()).append("\n");
        details.append("Eligibility Status : ").append(player.getMatchEligibilityStatus()).append("\n");
        details.append("Contact Number     : ").append(player.getContactNumber()).append("\n\n");

        details.append("MEDICAL FITNESS DECLARATIONS\n");
        details.append("=====================================================\n");

        if (declarations.isEmpty()) {
            details.append("No medical fitness declaration found for this player.");
        } else {
            int declarationNumber = 1;

            for (MedicalDeclaration declaration : declarations) {
                ...
                details.append("Declaration ID     : ")
                        .append(declaration.getDeclarationId())
                        .append("\n");

                details.append("Medical Officer ID : ")
                        .append(declaration.getMedicalOfficerId())
                        .append("\n");

                details.append("Match Date         : ")
                        .append(declaration.getDeclarationDate())
                        .append("\n");

                details.append("Declaration Status : ")
                        .append(declaration.getDeclarationStatus())
                        .append("\n");

                details.append("Remarks            : ")
                        .append(
                                declaration.getRemarks() == null
                                        || declaration.getRemarks().isBlank()
                                        ? "None"
        ...
        player_details_textarea.setText(details.toString());
    ...
    private boolean loadPlayersFromFile() {
        ...
        System.out.println(
                "Loading players from: " + playerFile.getAbsolutePath()
        );
        ...
        try (ObjectInputStream inputStream =
            ...
            System.out.println(
                    "Players loaded successfully: " + playerList.size()
            );
