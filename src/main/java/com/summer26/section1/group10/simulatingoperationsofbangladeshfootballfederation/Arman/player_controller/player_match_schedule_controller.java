...
public class player_match_schedule_controller {
    ...
    @FXML
    private TableView<Managematch>
            player_match_schedule_tableview;

    @FXML
    private TableColumn<Managematch, String>
            match_id_column;
    ...
    @FXML
    private TableColumn<Managematch, LocalDate>
            match_date_column;
    ...
    private void initializeTableColumns() {
        ...
        match_id_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "matchId"
                )
        );

        home_team_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "hometeam"
                )
        );

        away_team_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "awayteam"
                )
        );

        competition_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "competition"
                )
        );

        stadium_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "stadium"
                )
        );
        ...
        match_time_column.setCellValueFactory(
                new PropertyValueFactory<>(
                        "matchtime"
                )
        );
    ...
    private void loadAllMatches() {
        ...
        /*
         * LinkedHashMap prevents duplicate match IDs.
         *
         * Default matches are added first.
         * Saved matches from Managematches.bin are added later.
         *
         * Therefore, if the same match ID exists in the saved file,
         * the updated saved match replaces the default match.
         */
        Map<String, Managematch> combinedMatchMap =
                new LinkedHashMap<>();

        for (Managematch defaultMatch :
                createDefaultMatches()) {

            if (defaultMatch.getMatchId() != null) {

                combinedMatchMap.put(
                        defaultMatch.getMatchId(),
                        defaultMatch
                );
            }
        }
        ...
        List<Managematch> combinedMatches =
                new ArrayList<>(
                        combinedMatchMap.values()
                );

        /*
         * Sort matches by date.
         * Matches without a date are placed at the end.
         */
    ...
    private List<Managematch> createDefaultMatches() {
        ...
        List<Managematch> defaultMatches =
                new ArrayList<>();
